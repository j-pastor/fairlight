...
func _physics_process(_delta: float) -> void :
	...
	if step_status == 0  :
		...
		var has_intersect_step : Array = Mainglobal.check_intersection(self_id, obj["pos"]+Vector3(0,0,-6))
		
		if obj["is_falling"] : 
			print(has_intersect_step)
			if (has_intersect_step[0] and not has_intersect_step[3].begins_with("step-")) or not has_intersect_step[0] : sprite.stop()
